...
class Change_pw extends Component {
  ...
  forget() {
    ...
    var mobiles = this.refs.mobiles.value
    var us_pwd = this.refs.us_pwd.value, us_pwds = this.refs.us_pwds.value, code = this.refs.code.value
    const body = {
      n_pwd: us_pwd,
      o_pwd:mobile,
      smsCode:code,
    ...
    if (!mobile) { return Toast.info("请输入旧支付密码", 1) }
    if (!mobiles) { return Toast.info("请输入手机号", 1) }
    if (!code) { return Toast.info("请输入验证码", 1) }
    if (!us_pwd) { return Toast.info("请再次输入密码", 1) }
    if (us_pwd.length < 6) {
    ...
    user.editPayPwd(body).then(data => {
      if(data.code==1){
  ...
  forgetCode() {
    var mobile = this.refs.mobiles.value
    var data = {
      mobile
    }
    if (!mobile) {
      ...
      <div className="res">
        ...
        <div className="user-res-input">
          <div className="user-content">
            <div className="form"><span>旧支付密码</span> <input ref="mobile" placeholder="旧支付密码" /> </div>
            <div className="form"><span>手机号</span> <input ref="mobiles" placeholder="手机号" /> </div>
            ...
            <div className="form"><span>设置支付密码</span>  <input ref="us_pwd" placeholder="6-18位字符" /> </div>
            <div className="form"><span>确认支付密码:</span> <input ref="us_pwds" placeholder="请再次输入支付密码" /> </div>
